...
- `priority`: P1
- `status`: completed
- `implementation overview`: Encode analyst/architect/team-executor/code-simplifier/code-reviewer lanes, mandatory large-TODO subdivision, and per-TODO completion gates in skill text and contract tests.
- `acceptance`: The workflow explicitly enforces planning, decomposition, bounded execution, and final review sequencing.
- `evidence`: `oh-my-ralpha` now hard-requires plan-first, sub-TODO decomposition for large work, one `in_progress` slice, layered verify -> simplify -> regress -> artifact sync, and the default lane map `analyst` / `architect` / `team-executor` / `code-simplifier` / `code-reviewer`.
